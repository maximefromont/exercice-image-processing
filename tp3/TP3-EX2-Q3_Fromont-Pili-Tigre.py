...
src_pts = np.float32([kp1[m.queryIdx].pt for m in best_consistent_subset]).reshape(-1, 1, 2)
dst_pts = np.float32([kp2[m.trainIdx].pt for m in best_consistent_subset]).reshape(-1, 1, 2)

transformation_matrix, _ = cv2.estimateAffinePartial2D(src_pts, dst_pts)

print("Matrice de transformation optimale :")
